...
plt.savefig(fallback_filename+'_linear.png', dpi=200)

try:
    fig = plt.figure(2)
    plt.loglog(np.abs(voltages), np.abs(currents), marker="o", linestyle="None")
    plt.xlabel('|U| [V]')
    plt.ylabel('|I| [A]')
    plt.title(mid)
    plt.savefig(fallback_filename+'_loglog.png', dpi=200)
    plt.savefig(filename+'_loglog.png', dpi=200)
    plt.show()
except:
    print('loglog plot failed.')
    raise
