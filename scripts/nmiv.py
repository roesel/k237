...
if len(sys.argv) != 7:
    if sys.argv[1] == '--help':
        print(' Usage: python nmiv.py START_V STOP_V RETURN #_OF_POINTS #_OF_AVG NAME')
    else:
...
try:
    inst.write("B" + str(voltages[0]) + ",0,0X")   # Bias current , auto range, no delay
    inst.write("N1X")  # Operate
    # TODO Should we sleep here?
    for v in voltages:
        inst.write("B" + str(v) + ",0,0X")   # Bias current , auto range, no delay
        # TODO Should we sleep here?
        data = []
